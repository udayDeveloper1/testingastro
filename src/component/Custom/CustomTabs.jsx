...
import { closeLoder, formatDate, formatTime, openLoader } from '../../utils/CommonFunction'
...
const CustomTabs = () => {
  ...
  const KundliItem = JSON.parse(localStorage.getItem(Constatnt?.KUNDLI_KEY)) || null
  ...
  const kundliDetailsApiCalling = async (data, response) => {
    ...
    const handleSuccess = (key, value) => {
      ...
      const prev = JSON.parse(localStorage.getItem(Constatnt?.KUNDLI_KEY)) || {}
    ...
    try {
      ...
      if (panchangRes?.code === Codes.SUCCESS) {
        console.log('panchangRes data', panchangRes);
      ...
      const mahaDashaPrediction = await getMahaDashaPredictionTh(updatedRequest)
  ...
  useEffect(() => {
    if (!hasCalledApi.current && kundliDetailsData && Object.keys(KundliItem)?.length !== 24) {
      hasCalledApi.current = true
      openLoader(dispatch, 'freeKundli_details')
      console.log("kasjdjkasdkasd", !hasCalledApi.current, kundliDetailsData, KundliItem);

      kundliDetailsApiCalling(KundliItem?.panchangeDetails?.request, KundliItem?.panchangeDetails?.response)
  ...
  return (
    <>
      ...
      <CommonBanner
        // backgroundImage={currentTab?.bannerImage}
              ...
              return (
                <div
                  ...
                  onClick={() => {
                    ...
                    // kundaliDetailsNavigate(navigate, item.key, item, PATHS.FREEKUNDALI_DETAILS)
                  }}
                >
                  {t(`${item.key}`)}
                </div>
...
//       const panchangRes = await getPanchangTh(request)
//       console.log('panchangeDetails 123',panchangeDetails);
...
//   console.log('kundliDetailsData',kundliDetailsData);
